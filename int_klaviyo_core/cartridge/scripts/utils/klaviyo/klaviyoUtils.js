...
function trackEvent(email, data, event) {
    ...
    if (resultObj == 1) {
        ...
        logger.error('Klaviyo failed: ' + JSON.stringify(resultObj));
    ...
    for (var itemIndex = 0; itemIndex < basketItems.length; itemIndex++) {
        ...
        if (currentProductID != null && !empty(basketProduct) && basketProduct.getPriceModel().getPrice().value > 0) {
            ...
            if (basketProduct.variant) {
                primaryCategory = basketProduct.masterProduct && basketProduct.masterProduct.getPrimaryCategory() ? basketProduct.masterProduct.getPrimaryCategory().displayName : ''; // BBJ FIX
            } else {
                primaryCategory = basketProduct.getPrimaryCategory() ? basketProduct.getPrimaryCategory().displayName : '';  // BBJ FIX
            }
