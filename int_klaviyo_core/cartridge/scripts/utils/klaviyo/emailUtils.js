...
function prepareOrderPayload(order, isFutureOrder, mailType) {
    // TODO SAMI: Convert response object data to OrderModel (extended by multiple cartridges) vs Order Object
    // var orderModelObject = new OrderModel(order, {});
    ...
    if (order.shipments.length > 0) {
        ...
        if (giftCertificateLineItems && giftCertificateLineItems.length > 0) {
            ...
            for (var l in giftCertificateLineItems) {
                ...
                giftLineItemsArray.push({
                    ...
                    'Image'           : 'giftCertificateImage' in giftLineItem.custom && !empty(giftLineItem.custom.giftCertificateImage) ? giftLineItem.custom.giftCertificateImage : giftCardImage
                });
        ...
        orderDetails['Gift Certificate Present'] = giftLineItemsArray.length !== 0;

        // Push empty placeholder
        if (!orderDetails['Gift Certificate Present']) {
            giftLineItemsArray.push({
                'Recipient Name'  : '',
                'Recipient Email' : '',
                'Sender Name'     : '',
        ...
        for (var k in paymentInstruments) {
            ...
            if (paymentInstrumentItem.creditCardType) {
    ...
    billingaddress.push({
        ...
        'Postal Code'  : orderBillingAddressPostalCode,
